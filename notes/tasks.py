...
def send_welcome_email_task(self, user_id, note_title, note_content):
    try:
        ...
        html_content = render_to_string('ReminderAlert.html',
                                        {'username': User.objects.get(id=user_id).username, 'title': note_title, 'content': note_content})
...
def check_reminders_task(self):
    ...
    for reminder in reminder_list:
        ...
        print("The reminder notes check must happen every 1 second")
        if timedelta(seconds=12) < time_diff <= timedelta(seconds=15):
